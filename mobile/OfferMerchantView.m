...
-(void) updateView:(ttDealOffer *)newOffer merchant:(ttMerchant *)merch
{
    ...
    [merchantLogo sd_setImageWithURL:[NSURL URLWithString:merchant.closestLocation.logoUrl]
                     placeholderImage:[UIImage imageNamed:@"000.png"]
                            completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                                if (error !=  nil) {
                                    // TODO track these errors
    ...
    NSString *summary = [NSString stringWithFormat:@"%@ has made the following deal(s) available in %@.", merchant.name, offer.title];
